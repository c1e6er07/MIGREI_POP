        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid lg:grid-cols-2 gap-12">
                <div className="space-y-6">
                    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl relative overflow-hidden group hover:border-emerald-500/30 transition-all"> <div className="flex items-start gap-4"> <div className="p-3 bg-emerald-500/10 rounded-lg text-emerald-500"> <Phone className="w-6 h-6" /> </div> <div> <h3 className="font-bold text-white mb-1">Central de Atendimento</h3> <p className="text-slate-400 text-sm mb-2">Exclusivo para clientes e franqueados.</p> <p className="text-white font-mono text-lg">{COMPANY_INFO.whatsapp}</p> </div> </div> </div>
                ...
                <div>
                    <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 p-8 rounded-3xl">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="space-y-1"> <label className="text-xs font-bold text-slate-400 uppercase">Assunto</label> <select className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-3 text-white focus:border-emerald-500 outline-none transition-all text-sm" value={formData.subject} onChange={e => setFormData({...formData, subject: e.target.value})}> <option value="">Selecione um assunto...</option> <option value="Comercial">Quero contratar energia</option> <option value="Franquia">Interesse na Franquia</option> <option value="Suporte">Suporte Técnico</option> <option value="Outros">Outros assuntos</option> </select> </div>
