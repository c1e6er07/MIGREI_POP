                ...
                <div>
                    <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 p-8 rounded-3xl">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="space-y-1"> <label htmlFor="subject" className="text-xs font-bold text-slate-400 uppercase">Assunto</label> <select id="subject" className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-3 text-white focus:border-emerald-500 outline-none transition-all text-sm" value={formData.subject} onChange={e => setFormData({...formData, subject: e.target.value})}> <option value="">Selecione um assunto...</option> <option value="Comercial">Consultoria de Energia</option> <option value="Suporte">Suporte Técnico</option> <option value="Outros">Outros assuntos</option> </select> </div>
