...
const Finance: React.FC = () => {
  ...
  const [paymentStep, setPaymentStep] = useState<
    'select-method' | 'pix-code' | 'card-form' | 'processing' | 'success'
  >('select-method');
  ...
  const handleSelectMethod = async (method: PaymentMethod) => {
    ...
    if (method.id === 'pix' && selectedInvoice) {
      // generatePIXCode expects (invoiceId, amount)
      const code = PaymentService.generatePIXCode(selectedInvoice.id, selectedInvoice.total_value);
      setPixCode(code);
  ...
  const handleConfirmPayment = async () => {
    ...
    if (selectedMethod.id === 'pix' && pixCode) {
      const ok = await PaymentService.verifyPIXPayment(pixCode.copyPaste);
      // verifyPIXPayment returns boolean in the mock service
      setTransactionId(ok ? `PIX-${Date.now()}` : '');
    } else if (selectedMethod.id === 'credit-card') {
      // processCardPayment(invoiceId, amount, cardToken, installments)
      ...
      // processDebitPayment(invoiceId, amount, cardToken)
  ...
  const faqs = [
    {
      ...
      answer:
        'Gere o código QR ou copy-paste e escaneie no seu banco. O pagamento é processado em segundos, 24/7.',
    },
    {
      question: 'Posso pagar com cartão?',
      answer:
        'Sim! Você pode usar cartão de crédito em até 3 parcelas ou débito em conta para pagamento imediato.',
    },
    {
      question: 'Meus dados estão seguros?',
      answer:
        'Todos os pagamentos são processados com criptografia de banco. Nunca compartilhamos dados pessoais ou financeiros.',
    },
  ];
  return (
    <div className="space-y-8">
      ...
      <motion.div
        ...
        className="bg-gradient-to-r from-emerald-900/40 to-slate-900 border border-emerald-500/20 rounded-xl p-6 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden"
      >
        ...
        <div className="flex-1 z-10">
          ...
          <p className="text-slate-300 text-sm leading-relaxed">
            Realize pagamentos com PIX instantâneo ou cartão de crédito/débito. Tecnologia segura
            com criptografia bancária.
          </p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex border-b border-slate-800 mb-6">
            <button
              ...
              className={`pb-3 px-4 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'payable' ? 'border-emerald-500 text-emerald-500' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              {' '}
              <Banknote className="w-4 h-4" /> Contas a Pagar{' '}
              {invoices.length > 0 && (
            ...
            <button
              ...
              className={`pb-3 px-4 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'history' ? 'border-emerald-500 text-emerald-500' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              {' '}
              <History className="w-4 h-4" /> Hist├│rico de Transa├º├Áes{' '}
            </button>
          </div>
          {loading ? (
            ...
            invoices.length === 0 ? (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
                ...
                <p className="text-slate-400">
                  Voc├¬ n├úo possui faturas pendentes no momento.
                </p>{' '}
              </div>
            ) : (
              <div className="space-y-4">
                ...
                {invoices.map((inv) => (
                  <motion.div
                    key={inv.id}
                    className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 hover:border-emerald-500/30 transition-all"
                  >
                    {' '}
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                      ...
                      <div>
                        {' '}
                        <h4 className="font-bold text-white text-lg">Fatura de Energia</h4>{' '}
                    ...
                    <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
                      ...
                      <button
                        ...
                        className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg shadow-lg shadow-emerald-500/20 transition-all transform hover:scale-105"
                      >
                        {' '}
                        Pagar Agora{' '}
                      </button>{' '}
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Sem hist├│rico</h3>{' '}
            ...
            <div className="space-y-4">
              ...
              {paidInvoices.map((inv) => (
                <div
                  key={inv.id}
                  className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center justify-between opacity-75 hover:opacity-100 transition-opacity"
                >
                  {' '}
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      {' '}
                      <h4 className="font-bold text-white">Pagamento Realizado</h4>{' '}
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-slate-500 uppercase">Via Open Finance</p>{' '}
                  </div>{' '}
                </div>
              ))}{' '}
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
              {' '}
              <Lock className="w-5 h-5 text-slate-400" /> Central de Seguran├ºa{' '}
            </h3>{' '}
            <div className="space-y-4">
              {' '}
              {faqs.map((faq, idx) => (
                <div key={idx} className="border-b border-slate-800 last:border-0 pb-4 last:pb-0">
                  ...
                  <button
                    ...
                    className="w-full flex justify-between items-center text-left text-sm font-medium text-slate-300 hover:text-white transition-colors"
                  >
                    {' '}
                    {faq.question}{' '}
                    <ChevronDown
                      className={`w-4 h-4 transition-transform ${expandedFaq === idx ? 'rotate-180' : ''}`}
                    />{' '}
                  </button>{' '}
                  <AnimatePresence>
                    {' '}
                    {expandedFaq === idx && (
                      ...
                      >
                        {' '}
                        <p className="text-xs text-slate-500 mt-2 leading-relaxed pl-2 border-l-2 border-emerald-500/30">
                          {' '}
                          {faq.answer}{' '}
                        </p>{' '}
                      </motion.div>
          ...
          <div className="bg-gradient-to-br from-emerald-900/20 to-slate-900 border border-emerald-500/20 rounded-xl p-6">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-bold text-white">Processamento Seguro</h3>{' '}
            </div>{' '}
            <p className="text-xs text-slate-400 mb-4">
              {' '}
              Todas as transa├º├Áes s├úo processadas atrav├®s do Open Finance, garantindo m├íxima
              seguran├ºa e conformidade regulat├│ria.{' '}
            </p>{' '}
          </div>
        </div>
      </div>
              ...
              <div className="p-6">
                    ...
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="text-xl font-bold text-emerald-400">
                          {formatMoney(selectedInvoice.total_value)}
                        </span>
                      </div>
                ...
                {paymentStep === 'card-form' && (
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="text-sm text-slate-400 mb-2 block">Número do Cartão</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="text-sm text-slate-400 mb-2 block">Nome no Cartão</label>
                      <input
                        type="text"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-sm text-slate-400 mb-2 block">Validade</label>
                        <input
                          type="text"
                          placeholder="MM/AA"
                          className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-lg text-white"
                      ...
                      <div>
                        <label className="text-sm text-slate-400 mb-2 block">CVV</label>
                        <input
                          type="text"
                          placeholder="123"
                          className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-lg text-white"
                    ...
                    {selectedMethod?.id === 'credit-card' && (
                      <div>
                        <label className="text-sm text-slate-400 mb-2 block">Parcelas</label>
                        <select
                        ...
                        >
                          <option value="1">
                            1x de {formatMoney(selectedInvoice.total_value)}
                          </option>
                          <option value="2">
                            2x de {formatMoney(selectedInvoice.total_value / 2)}
                          </option>
                          <option value="3">
                            3x de {formatMoney(selectedInvoice.total_value / 3)}
                          </option>
                        </select>
                      </div>
                    )}
                ...
                {paymentStep === 'success' && (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-2xl font-bold text-white mb-2">Pagamento Realizado!</h3>
