    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[
          ...
          <div key={i} className="bg-slate-900 border border-slate-800 p-6 rounded-xl hover:border-emerald-500/50 transition-all group cursor-pointer">
             <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-xs font-bold bg-slate-950 px-2 py-1 rounded text-slate-400">{rep.type}</span>
             </div>
             <h3 className="text-lg font-bold text-white mb-1">{rep.title}</h3>
             <p className="text-sm text-slate-500 mb-4 flex items-center gap-1"><Calendar className="w-3 h-3" /> {rep.date}</p>
