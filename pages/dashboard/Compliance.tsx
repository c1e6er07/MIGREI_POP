  ...
  const regulatoryStatus = [
    {
      agency: 'ANEEL',
      status: 'compliant',
      lastCheck: 'Hoje, 08:00',
      message: 'Resolução 1.000/2021 atendida.',
    },
    {
      agency: 'CCEE',
      status: 'compliant',
      lastCheck: 'Hoje, 08:15',
      message: 'Adesão e modelagem regular.',
    },
    {
      agency: 'BACEN',
      status: 'warning',
      lastCheck: 'Ontem',
      message: 'Nova circular sobre ITP pendente de revisão.',
    },
    {
      agency: 'LGPD',
      status: 'compliant',
      lastCheck: 'Contínuo',
      message: 'Auditoria de dados ativa.',
    },
  ];
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Monitoramento regulatório e integridade corporativa.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {regulatoryStatus.map((item, idx) => (
              ...
              >
                ...
                <p
                  className={`text-sm font-medium mb-1 ${item.status === 'compliant' ? 'text-emerald-400' : 'text-orange-400'}`}
                >
                  {item.status === 'compliant' ? 'Regular' : 'Atenção Requerida'}
                </p>
                <p className="text-xs text-slate-500">{item.message}</p>
                <p className="text-[10px] text-slate-600 mt-4 pt-2 border-t border-slate-800/50">
          ...
          <div className="bg-red-900/10 border border-red-500/20 rounded-2xl p-8">
            ...
            <div className="space-y-4">
              <p className="text-slate-400 text-sm leading-relaxed">
                Este canal deve ser utilizado para reportar violações ao Código de Ética, corrupção,
                assédio, fraudes financeiras ou irregularidades regulatórias na rede MIGREI
                CORPORATION.
              </p>
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                ...
                <select
                  ...
                  className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg px-4 py-3 outline-none focus:border-red-500"
                >
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                ...
                <textarea
                  className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg px-4 py-3 outline-none focus:border-red-500 h-32 resize-none"
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <label htmlFor="anon" className="text-sm text-slate-400 select-none cursor-pointer">
                  Desejo manter meu anonimato total.
                </label>
              </div>
