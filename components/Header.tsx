...
const Header: React.FC = () => {
  ...
  const NavLink = ({ to, label }: { to: string; label: string }) => (
    <Link
      to={to}
      className={`relative text-sm font-medium transition-colors hover:text-white group ${isActive(to) ? 'text-emerald-400' : 'text-slate-400'}`}
    >
      {label}{' '}
      <span
        className={`absolute -bottom-1 left-0 w-0 h-0.5 bg-emerald-500 transition-all group-hover:w-full ${isActive(to) ? 'w-full' : ''}`}
      ></span>
    </Link>
  );
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-950/90 backdrop-blur-xl border-b border-slate-800 shadow-lg">
        ...
        <div className="flex items-center justify-between h-20">
          <Link to="/" className="flex items-center gap-3 group shrink-0">
            ...
            <div className="flex flex-col">
              {' '}
              <span className="text-lg font-display font-bold text-white tracking-wide group-hover:text-emerald-400 transition-colors">
                MIGREI
              </span>{' '}
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            >
              ...
              <div className="relative flex items-center z-10">
                {' '}
                <User className="w-4 h-4 mr-2" /> Portal do Cliente{' '}
              </div>{' '}
            </Link>
          </div>
      ...
      {isOpen && (
        <div className="lg:hidden bg-slate-900 border-b border-slate-800 animate-in slide-in-from-top-5">
          {' '}
          <div className="px-4 pt-4 pb-6 space-y-2">
            ...
            <Link
              to="/login"
              onClick={() => setIsOpen(false)}
              className="block w-full text-center px-4 py-4 rounded-xl text-base font-bold text-slate-950 bg-gradient-to-r from-orange-400 to-yellow-400 hover:from-orange-500 hover:to-yellow-500 transition-colors"
            >
              {' '}
